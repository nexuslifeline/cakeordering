...
class Cake_layer_1_Controller extends CORE_Controller
{
    ...
    public function transaction($txn = null)
    {
        switch ($txn) {
            ...
            case 'create':
                $m_cake_layer_1= $this->Cake_layer_1_model;
         
                $m_cake_layer_1->begin();
                $m_cake_layer_1->cake_description = $this->input->post('cake_description', TRUE);
                $m_cake_layer_1->image_path = sha1($this->input->post('image_path', TRUE));
         
                
                // auditing purposes
                
                $m_cake_layer_1->save();
                $cake_id = $m_cake_layer_1->last_insert_id();
                $m_cake_layer_1->commit();
                if ($m_cake_layer_1->status() === TRUE) {
            ...
            case 'delete':
                ...
                $cake_id        = $this->input->post('cake_id', TRUE);
                $m_cake_layer_1->begin();
                $m_cake_layer_1->is_deleted = 1;
                
                $m_cake_layer_1->modify($cake_id);
                
                // make sure to update status of the cust
                
                $m_cake_layer_1->is_active = 0;
                $m_cake_layer_1->modify($cake_id);
                $m_cake_layer_1->commit();
                if ($m_cake_layer_1->status() === TRUE) {
                    $response['title'] = 'Success!';
   ...
   private  function get_response_rows($id=null){
        ...
        return  $m_cake_layer_1->get_list(
