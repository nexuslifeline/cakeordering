  $(document).ready(function() {
      loadItem();
      var _txnMode, graphic_id, oSelecteditem;
      ...
      $(document).on('click', '[name="edit_info"]', function() {
          ...
          graphic_id = oSelecteditem.find('[name="graphic_id"]').val();
             ...
             _data.push({
              ...
              if (_txnMode == "new") {
                  $.ajax({
                      "dataType": "json",
                      "type": "POST",
                      "url": http + "User_graphics/transaction/create",
                      "data": _data,
                      "beforeSend": function() {
  ...
  _data.push({
                 ...
                 value: localStorage.customer_id
                  ...
                  $.ajax({
                      "dataType": "json",
                      "type": "POST",
                      "url": http + "User_graphics/transaction/update",
                      "data": _data,
                      "beforeSend": function() {
      ...
      $('#confirm_delete').click(function() {
          ...
          $.ajax({
              "dataType": "json",
              "type": "POST",
              "url": http + "User_graphics/transaction/delete",
              "data": [{
                  name: "graphic_id",
                  value: graphic_id
              }]
  ...
  function createStructure(value) {
      var tags =
          ...
          '<input type="hidden" name="graphic_id" value="' + value.graphic_id + '" />' +
          ...
          '<h4 class="group inner list-group-item-heading" name="graphic_name">' + value.graphic_name + '</h4>' +
          '<p class="group inner list-group-item-text" name="graphic_description">' + value.graphic_description +
          '<div class="row">' +
  ...
  function loadItem() {
      ...
      $.ajax({
          dataType: "json",
          type: "POST",
          url: http + 'User_graphics/get_customer_graphics/'+localStorage.customer_id,
      ...
      $('input[name="file_upload[]"]').change(function(event) {
          ...
          $.ajax({
              url: http + 'User_graphics/transaction/upload',
              type: "POST",
              data: data,
              cache: false,
              dataType: 'json',
