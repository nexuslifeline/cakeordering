  $(document).ready(function() {
      loadItem();
      var _txnMode, flavor_id, oSelecteditem;
      ...
      $(document).on('click', '[name="edit_info"]', function() {
          ...
          flavor_id = oSelecteditem.find('[name="flavor_id"]').val();
      ...
      $('#btn_save').click(function() {
          ...
          if (validateRequiredFields(f)) {
              ...
              if (_txnMode == "new") {
                  $.ajax({
                      "dataType": "json",
                      "type": "POST",
                      "url": http + "Flavors/transaction/create",
                      "data": _data,
                      "beforeSend": function() {
          ...
          $('#top_view').attr('src', '../../assets-apps/img/dummy/image-upload-placeholder.jpg');
                  ...
                  $.ajax({
                      "dataType": "json",
                      "type": "POST",
                      "url": http + "Flavors/transaction/update",
                      "data": _data,
                      "beforeSend": function() {
      ...
      $('#confirm_delete').click(function() {
          ...
          $.ajax({
              "dataType": "json",
              "type": "POST",
              "url": http + "Flavors/transaction/delete",
              "data": [{
                  name: "flavor_id",
                  value: flavor_id
              }]
  ...
  function createStructure(value) {
      var tags =
          ...
          '<input type="hidden" name="flavor_id" value="' + value.flavor_id + '" />' +
          '<img class="group list-group-image img-thumbnail" title="'+ value.flavor_description +'" name="image_path" src="' + value.image_path + '" alt="">' +
           '<input type="hidden" name="top_view" value="' + value.top_view + '" />' +
          '<div class="caption" style="border: solid #ec4444 1px;"  >' +
          '<h4 class="group inner list-group-item-heading" name="flavor_name">' + value.flavor_name + '</h4>' +
       
          '<div class="row">' +
          '<div class="col-xs-12 col-md-12">' +
          '<p  > PHP.<span name="price">' + value.price + '</span></p>' +
            '<p name="flavor_description" style="display:none;">' + value.flavor_description +  '</span></p>' +
          '</div>' +
  ...
  function loadItem() {
      ...
      $('input[name="file_upload[]"]').change(function(event) {
          ...
          $.ajax({
              url: http + 'Flavors/transaction/upload',
              type: "POST",
              data: data,
              cache: false,
              dataType: 'json',
